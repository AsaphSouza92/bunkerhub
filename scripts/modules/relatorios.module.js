...
export async function listarRelatorios() {
  const [relatorios, eventos] = await Promise.all([
    relatoriosRepository.listar(),
    listarEventos()
  ]);
...
export async function eventosSemRelatorio() {
  ...
  const idsComRelatorio = new Set(
    relatorios.map(r => r.evento_id || r.eventoId)
  );
...
export async function criarRelatorio(dados) {
  if (!dados.eventoId) {
    throw new Error(
      'Selecione o evento ao qual este relatório se refere.'
    );
  }

  if (await relatoriosRepository.buscarPorEvento(dados.eventoId)) {
    throw new Error(
      'Este evento já possui um relatório.'
    );
  }
