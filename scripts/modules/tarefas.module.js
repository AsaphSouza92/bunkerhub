...
export async function listarTarefas({ apenasPendentes = false, apenasMinhas = false, usuarioId = 'mock-user' } = {}) {
  const tarefas = await tarefasRepository.listar(t => {
    const bateConcluida = apenasPendentes ? !t.concluida : true;
    const bateResponsavel = apenasMinhas ? t.responsavelId === usuarioId : true;
    return bateConcluida && bateResponsavel;
  });
...
export async function criarTarefa(dados) {
  if (!dados.titulo || dados.titulo.trim().length < 3) throw new Error('Descreva a tarefa com um pouco mais de detalhe.');
...
export async function alternarConclusao(id) {
  ...
  const atualizada = await tarefasRepository.atualizar(id, { concluida: !tarefa.concluida });
