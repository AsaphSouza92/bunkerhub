...
export async function buscarPessoaPorId(id) {
  return pessoasRepository.buscarPorId(id);
}

export async function pessoasComAcompanhamentoAtrasado() {
  const hoje = new Date().toISOString().slice(0, 10);

  return pessoasRepository.listar(
    p =>
      p.proximoAcompanhamento &&
      p.proximoAcompanhamento <= hoje
  );
}
...
export async function salvarPessoa(dados, id = null) {
  ...
  const pessoa = id
    ? await pessoasRepository.atualizar(id, dados)
    : await pessoasRepository.criar(dados);
...
export async function registrarAcompanhamento(
  ...
  await pessoasRepository.adicionarHistorico(
    id,
    nota
  );

  await pessoasRepository.atualizar(id, {
    proximoAcompanhamento: proximaData || null
  });
