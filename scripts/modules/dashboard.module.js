...
export async function getDashboardData() {
  const [pessoas, eventos, tarefasPendentes, versiculo] = await Promise.all([
  ...
  return {
    versiculo,
    proximoEvento: eventos.sort((a, b) => new Date(a.data) - new Date(b.data))[0] || null,
    aniversariantes: proximoAniversario(pessoas.filter(p => p.nascimento)),
    tarefasPendentes,
  };
}
