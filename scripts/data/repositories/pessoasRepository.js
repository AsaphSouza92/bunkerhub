...
function paraSupabase(dados) {
  ...
  if (dados.nascimento !== undefined) payload.nascimento = dados.nascimento || null;
  ...
  if (dados.dataEntrada !== undefined || dados.data_entrada !== undefined) {
    payload.data_entrada = dados.data_entrada || dados.dataEntrada;
  }
  if (dados.observacoes !== undefined) payload.observacoes = dados.observacoes;
  if (dados.proximoAcompanhamento !== undefined || dados.proximo_acompanhamento !== undefined) {
    payload.proximo_acompanhamento = dados.proximo_acompanhamento ?? dados.proximoAcompanhamento ?? null;
  }
...
function paraApp(pessoa) {
  ...
  return {
    ...
    proximoAcompanhamento: pessoa.proximo_acompanhamento ?? pessoa.proximoAcompanhamento ?? null,
...
export const pessoasRepository = {
  ...
  async criar(dados) {
    return paraApp(await db.create(COLLECTION, paraSupabase(dados)));
  },

  async atualizar(id, patch) {
    return paraApp(await db.update(COLLECTION, id, paraSupabase(patch)));
  },
